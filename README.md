...
但目前正式評級門檻不是直接用 15 項數量決定，而是共同檢查評級勝率、EV 與下面的五項方向。
...
評級勝率 < 65%
→ 不得進入 A 或 B
→ 即使 EV 與五項原本達到 A／B，最高只能降為 C
```

此門檻只限制 A／B：

- 不會把原本的 C 升級。
- 不會救回 EV≤0 或五項≤2／5 的淘汰場次。
- 65.00% 視為通過；64.99% 視為未通過。

設定位置：

```text
ratio_config.json
rating.AB_probability_min_pct
```
...
```text
A＝勝率≥65% 且 EV≥7% 且五項5/5
B＝勝率≥65% 且 EV≥4% 且五項≥4/5，且未達A
C＝EV>0% 且五項≥3/5，但未達A／B完整條件
淘汰＝EV≤0% 或五項≤2/5
```
...
D 值看數據差距與方向
排名情境修正排名作用
評級勝率將市場、數據與排名整合，並作為 A／B 的 65% 最低門檻
EV 決定價格是否值得
五項支持限制最高評級
```
...
> **15 項看得廣；5 項定方向；D 看差距；EV 定價格；A／B 再守 65% 勝率門檻。**
...
`AB_probability_min_pct` 只限制 A／B；低於此值時最高為 C。
...
Formula B v1.3
BO3 Mechanical v1.0
APP_VERSION 4.4.10-rating65-ab-gate
